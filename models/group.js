...
class Grupo {
  ...
  static async addParticipante(idGrupo, idUsuario) {
    ...
    if (grupo.status !== "aberto") throw new Error("Grupo não está aberto para novos participantes.");
    if (grupo.participantes.includes(idUsuario)) throw new Error("Usuário já está no grupo");
  ...
  static async realizarSorteio(idGrupo) {
    ...
    if (!grupo) throw new Error("Grupo não encontrado...");
    if (grupo.participantes.length < 3) throw new Error("É necessário pelo menos 3 participantes para realizar o sorteio");
  ...
  static async buscarGruposDoUsuario(idUsuario) {
    ...
    return grupos.filter(
      (grupo) => grupo.participantes.includes(idUsuario) || grupo.idAdmin === idUsuario
    );
  }
}
